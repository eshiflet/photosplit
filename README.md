...
| Also heal the dirt the last calibration found | Post-Processing. Needs a calibration from the last week, not a resolution: the position is known rather than detected, so it works at any dpi and cannot mistake the photograph for dirt |
...
calibration is more than a week old, because after a clean the map is a list of
places with nothing in them. Recalibrate whenever you clean the glass and it
stays current.
